      ...
      <svg viewBox={`0 0 ${SVG_W} ${SVG_H}`} className="obs-svg">
        ...
        <g clipPath="url(#clip-ml-mnv)">
          ...
          {sorted.map((e, i) => {
            ...
            return isDetected ? (
              <g key={i}>
                <line x1={cx.toFixed(1)} y1={P.top} x2={cx.toFixed(1)} y2={P.top + innerH} stroke={eventColor} strokeWidth="1.5" opacity="0.3" />
                <circle cx={cx} cy={cy} r="6" fill={eventColor} opacity="0.9" />
                <text x={cx} y={cy - 10} textAnchor="middle" fontSize="9" fontWeight="700" fill={eventColor}>
                  {e.delta_v_m_s.toFixed(1)}
                </text>
              </g>
            ) : (
              <circle key={i} cx={cx} cy={cy} r="2.5" fill={baseColor} opacity="0.5" />
            )
          })}
        </g>
      </svg>
