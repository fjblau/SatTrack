...
export default function NextPassContent({ resolvedTarget }) {
  ...
  const fetchPasses = useCallback(async () => {
    ...
    try {
      let res
      if (resolvedTarget.type === 'tle') {
        res = await fetch('/v2/public/passes/tle', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            tle_text: resolvedTarget.tleText,
            lat: parsedLat,
            lon: parsedLon,
        ...
        res = await fetch(`/v2/public/passes/${resolvedTarget.noradId}?${params}`)
      }
  ...
  }, [lat, lon, elevationM, minElevDeg, hoursAhead, resolvedTarget])
